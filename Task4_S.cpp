...
public:
    ...
    void run(vector<Point>& all_points){
        ...
        //Write points in clusters to file
        ofstream outfile;
        outfile.open("output.txt");
        if(outfile.is_open()){
            for (int i=0; i<K; i++){
                outfile<<"Points in cluster "<<clusters[i].getId()<<" : \n";
            ...
            cout<<"Error: Unable to write to output.txt";
        }
...
int main(int argc, char **argv){
    ...
    cout<<"\nExecution time (seconds): "<<std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count()<<"\n";
